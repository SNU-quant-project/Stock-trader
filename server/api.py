...
def _fitness(ann_returns, sharpe, turnover):
    ...
    여기서 Returns 는 *연율화* 수익률 (기간 길이에 안 흔들리도록).
    Sharpe 는 부호 유지(나쁜 알파는 음수), turnover 하한 0.125.
    """
    import math
    tv = max(abs(turnover), 0.125)
    return round(sharpe * math.sqrt(abs(ann_returns) / tv), 2)


def _year_rows_from_returns(rets, m):
    """returns Series 를 연도별로 집계. fitness 는 연율화 수익률로 계산
    (표의 Returns 컬럼은 그 기간 누적, fitness 입력은 연율화 — Brain 동일)."""
    import numpy as np
    rows = []
    s = rets.copy()
    ...
    return rows or [{
        ...
        "turnover": m["avg_turnover"], "fitness": _fitness(m["annual_return"], m["sharpe"], m["avg_turnover"]),
