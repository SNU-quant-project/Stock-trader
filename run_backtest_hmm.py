...
def parse_args():
    ...
    _default_asset_name = _asset_name_from_path(config.DATA_PATH)
    p.add_argument('--asset-name', default=_default_asset_name,
                   help=f"차트 제목용 자산 이름 (기본: config.DATA_PATH에서 자동 추출 → '{_default_asset_name}')")
    ...
    p.add_argument('--test-end',    default='2026-05-22')
    ...
    # 출력
    _default_asset_slug = _default_asset_name.split('/')[0]  # 'BTC/USDT' → 'BTC'
    p.add_argument('--output-html', default=f'backtest_hmm_{_default_asset_slug}.html',
                   help=f"시각화 HTML 출력 경로 (기본: 'backtest_hmm_{{자산심볼}}.html')")
...
VARIANT_LIST = [
    ('variant_A', "HMM✓ Smooth✓",  True,  True),
    ('variant_B', "HMM✓ Smooth✗",  True,  False),
    ('variant_C', "HMM✗ Smooth✓",  False, True),
    ('variant_D', "HMM✗ Smooth✗",  False, False),
]
...
def run_hmm_variants(df_train, df_test, args):
    ...
    for variant_id, variant_label, inc_hmm, use_smooth in VARIANT_LIST:
...
def main():
    ...
    # 4. OOS 통계 계산
    summary_for_print = []
    for vid, vlabel, _, _ in VARIANT_LIST:
        r = hmm_results[vid]
