...
def _positions_dict(trading_client):
    return {
...
def _coerce_logged_target(value):
    if value is None or pd.isna(value):
        return None
...
def _latest_logged_expected_positions(
    symbols,
    log_path=LOG_PATH,
    min_expected=None,
):
    """최근 live_log 중 목표 포지션이 충분히 있는 사이클을 찾는다."""
    min_expected = (
        getattr(config, "LIVE_POSITION_SNAPSHOT_MIN_EXPECTED", 10)
    ...
    try:
        ...
        print(f"  [guard:position_snapshot] live_log 읽기 실패: {exc}")
        return {}, ""

    required_cols = {"timestamp", "symbol", "target"}
    if df.empty or not required_cols.issubset(df.columns):
        return {}, ""

    timestamps = df["timestamp"].dropna().drop_duplicates().tolist()
    symbol_set = set(symbols)
    for stamp in reversed(timestamps):
        group = df[df["timestamp"].eq(stamp)]
        expected = {}
        for _, row in group.iterrows():
            sym = row.get("symbol")
            if sym not in symbol_set:
                continue
            target = _coerce_logged_target(row.get("target"))
            if target is not None:
                expected[sym] = target
        nonzero = sum(1 for qty in expected.values() if int(round(qty)) != 0)
        if nonzero >= min_expected:
            return expected, str(stamp)

    return {}, ""


def _check_position_snapshot(positions, expected_positions, expected_source=""):
    if not getattr(config, "LIVE_POSITION_SNAPSHOT_GUARD", True):
        return True, "position snapshot guard disabled", ""

    min_expected = getattr(config, "LIVE_POSITION_SNAPSHOT_MIN_EXPECTED", 10)
    min_coverage = getattr(config, "LIVE_POSITION_SNAPSHOT_MIN_COVERAGE", 0.70)
    expected_nonzero = {
        sym: int(round(qty))
        for sym, qty in expected_positions.items()
        if int(round(qty)) != 0
    }
    expected_count = len(expected_nonzero)
    if expected_count < min_expected:
        return (
            True,
            f"prior target snapshot 없음/작음 ({expected_count} < {min_expected})",
            f"source={expected_source or 'n/a'}; expected_nonzero={expected_count}",
        )

    actual_on_expected = {
        sym: int(round(positions.get(sym, 0.0)))
        for sym in expected_nonzero
        if int(round(positions.get(sym, 0.0))) != 0
    }
    actual_count = len(actual_on_expected)
    coverage = actual_count / expected_count if expected_count else 1.0
    missing = [sym for sym in expected_nonzero if sym not in actual_on_expected]
    detail = (
        f"source={expected_source or 'n/a'}; "
        f"expected_nonzero={expected_count}; "
        f"actual_expected_nonzero={actual_count}; "
        f"actual_total_nonzero="
        f"{sum(1 for qty in positions.values() if int(round(qty)) != 0)}; "
        f"missing={','.join(missing[:12])}"
    )
    if len(missing) > 12:
        detail += f",...+{len(missing) - 12}"

    if coverage + 1e-12 < min_coverage:
        return (
            False,
            f"position snapshot coverage {actual_count}/{expected_count}="
            f"{coverage:.1%} < required {min_coverage:.1%}",
            detail,
        )
    return (
        True,
...
def _calendar_entry_value(entry, field):
    return entry.get(field) if isinstance(entry, dict) else getattr(entry, field, None)


def _calendar_entry_date(entry):
    value = _calendar_entry_value(entry, "date")
    if value is None:
        raise ValueError(f"calendar entry date 없음: {entry!r}")
    return pd.Timestamp(value).date()


def _calendar_entry_close_et(entry):
    session_date = _calendar_entry_date(entry)
    value = _calendar_entry_value(entry, "close")
    if value is None:
        raise ValueError(f"calendar entry close 없음: {entry!r}")

    if isinstance(value, dt_time):
        close_ts = pd.Timestamp(dt_datetime.combine(session_date, value))
    elif isinstance(value, str) and "T" not in value and "-" not in value:
        close_ts = pd.Timestamp(f"{session_date} {value}")
    else:
        close_ts = pd.Timestamp(value)

    if close_ts.tzinfo is None:
        return close_ts.tz_localize(MARKET_TZ)
    return close_ts.tz_convert(MARKET_TZ)


def _previous_trading_day(trading_client, date_value, lookback_days=14):
    as_of = pd.Timestamp(date_value).date()
    ...
    trading_days = sorted(
        day for day in (_calendar_entry_date(entry) for entry in calendar)
        if day < as_of
    )
    if not trading_days:
        raise RuntimeError(f"직전 거래일을 찾지 못함: {start}..{end}")
    return trading_days[-1]


def _latest_closed_trading_session(trading_client, now_et, lookback_days=14):
    now = _to_et_aware(now_et)
    start = (now.normalize() - pd.Timedelta(days=lookback_days)).date()
    end = now.date()
    ...
    for entry in calendar:
        close_et = _calendar_entry_close_et(entry)
        if close_et <= now:
            closed_sessions.append((_calendar_entry_date(entry), close_et))

    if not closed_sessions:
        raise RuntimeError(f"닫힌 거래 세션을 찾지 못함: {start}..{end}")

    return max(closed_sessions, key=lambda item: item[1])


def _check_eod_data_freshness(
    status_by_symbol,
    symbols,
    session_date,
    session_close_et,
    ...
    expected_last_bar = (
        _to_et_naive(session_close_et) - pd.Timedelta(minutes=BAR_MINUTES)
    )
    tolerance = pd.Timedelta(
        minutes=getattr(config, "LIVE_EXPECTED_BAR_TOLERANCE_MIN", 5))
    reasons = []
    ...
    for sym in symbols:
        ...
        if last_bar is None:
            reasons.append(f"{sym}: last bar missing")
        elif last_bar.date() != session_date:
            reasons.append(
                f"{sym}: last_bar date {last_bar.date()} "
                f"!= latest closed session {session_date}"
            )
        elif abs(last_bar - expected_last_bar) > tolerance:
            reasons.append(
                f"{sym}: last_bar {_format_bar(last_bar)} "
                f"!= expected EOD {_format_bar(expected_last_bar)}"
            )
    ...
    if reasons:
        ...
        return False, detail
    return True, (
        f"EOD data fresh through {_format_bar(expected_last_bar)} ET "
...
def _check_manifest_freshness(manifest, as_of_date, required_date=None):
    required_date = required_date or _previous_weekday(as_of_date)
...
def _read_latest_approved_manifest(require_fresh=True, trading_client=None):
    """latest approved manifest를 읽고, 필요하면 freshness를 검증한다."""
    ...
    if require_fresh:
        as_of_date = pd.Timestamp.now(tz=MARKET_TZ).date()
        required_date = None
        if trading_client is not None:
            try:
                required_date = _previous_trading_day(trading_client, as_of_date)
            except Exception as exc:
                print(
                    "  [경고] Alpaca calendar 조회 실패 — "
                    f"weekday 기준으로 artifact freshness 확인: {exc}"
                )
...
def load_approved_live_artifacts(require_fresh=True, trading_client=None):
    """latest approved model/beta manifest를 로드한다."""
    manifest = _read_latest_approved_manifest(
...
def _manifest_identity(manifest):
    ...
    return (
        ...
        manifest.get("beta_asof_date"),
        manifest.get("created_at"),
    )


def reload_approved_live_artifacts_if_needed(
    trading_client,
    strategies,
    histories,
    approved_beta_map,
    ...
    """루프 중 latest approved artifact가 바뀌면 즉시 다시 로드한다.

    execute 모드는 여기서도 freshness를 재검사한다. EOD job이 실패해 latest
    manifest가 전 거래일까지 갱신되지 않았다면 run_cycle로 들어가지 않는다.
    """
    require_fresh = (
        not dry_run or getattr(config, "LIVE_REQUIRE_APPROVED_MODEL", True)
    )
    latest_manifest = _read_latest_approved_manifest(
        require_fresh=require_fresh,
        trading_client=trading_client,
    )

    current_id = _manifest_identity(approved_manifest)
    latest_id = _manifest_identity(latest_manifest)
    if current_id == latest_id:
        return strategies, histories, approved_beta_map, approved_manifest

    old_run = None if approved_manifest is None else approved_manifest.get("run_id")
    print(
        "  [artifact reload] latest approved model/beta 재로드: "
        f"{old_run} → {latest_manifest.get('run_id')}",
        flush=True,
    )
    return load_approved_live_artifacts(
        require_fresh=require_fresh,
        trading_client=trading_client,
    )
...
def run_eod_training(data_client, trading_client=None, lookback_years=5):
    ...
    try:
        if trading_client is None:
            raise RuntimeError("trading_client 없음")
        session_date, session_close_et = _latest_closed_trading_session(
            trading_client,
            pd.Timestamp.now(tz=MARKET_TZ),
        )
        fresh_ok, fresh_reason = _check_eod_data_freshness(
            status_by_symbol,
            freshness_symbols,
            session_date,
            session_close_et,
        ...
        if not fresh_ok:
            ...
            print(f"  [EOD freshness] {fresh_reason}")
    except Exception as exc:
        train_failures["freshness"] = f"EOD freshness/calendar 조회 실패: {exc}"
...
def run_cycle(trading_client, data_client, strategies, histories, dry_run,
    ...
    positions = _positions_dict(trading_client)
    expected_positions, expected_source = _latest_logged_expected_positions(SYMBOLS)
    position_ok, position_reason, position_details = _check_position_snapshot(
        positions, expected_positions, expected_source)
    if not position_ok:
        retry_count = (
            0 if dry_run else getattr(config, "LIVE_POSITION_SNAPSHOT_RETRY_COUNT", 2)
        )
        retry_sleep = getattr(config, "LIVE_POSITION_SNAPSHOT_RETRY_SLEEP_SEC", 1.0)
        for attempt in range(1, retry_count + 1):
            print(
                f"  [guard:position_snapshot] 재조회 {attempt}/{retry_count} "
                f"({retry_sleep:.1f}s 대기)",
                flush=True,
            )
            time.sleep(retry_sleep)
            retry_positions = _positions_dict(trading_client)
            retry_ok, retry_reason, retry_details = _check_position_snapshot(
                retry_positions, expected_positions, expected_source)
            positions = retry_positions
            position_ok = retry_ok
            position_reason = retry_reason
            position_details = retry_details
            if retry_ok:
                position_reason = f"{retry_reason} after retry {attempt}"
                break

    _record_guard(
        guard_rows, cycle_id, stamp, mode, "position_snapshot",
        position_ok, position_reason, position_details,
    )
    if not dry_run and not position_ok:
        reason = "position snapshot guard failed"
        _append_guard_log(guard_rows)
        ...
        try:
            ...
            if approved_manifest is not None:
                (
                    ...
                    approved_beta_map,
                    approved_manifest,
                ) = reload_approved_live_artifacts_if_needed(
                    trading_client,
                    strategies,
                    histories,
                    approved_beta_map,
