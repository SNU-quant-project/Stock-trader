...
────────────────────────────────────────────────────────────────────
  ...
  [그래프 2] HMM Variant A: HMM✓ Smooth✓ — Bull/Side/Bear 음영 + equity
  [그래프 3] HMM Variant B: HMM✓ Smooth✗
  [그래프 4] HMM Variant C: HMM✗ Smooth✓
  [그래프 5] HMM Variant D: HMM✗ Smooth✗
...
REGIME_COLORS = {
    0: 'rgba(46, 204, 113, 0.10)',   # Bull — 초록
    1: 'rgba(149, 165, 166, 0.06)',  # Side — 회색
    2: 'rgba(231, 76, 60, 0.10)',    # Bear — 빨강
}
...
# ─────────────────────────────────────────────────────────────────
# HMM 라벨 inference (variant 시각화용)
# ─────────────────────────────────────────────────────────────────

def _infer_regime_labels(strategy, df_test: pd.DataFrame) -> np.ndarray:
    """
    학습 완료된 HMMStrategy로 df_test의 각 봉에 대해 HMM 라벨(0/1/2) 산출.

    내부적으로 HMMStrategy의 파이프라인 그대로 재실행.
    """
    ...
    features = compute_window_features(
        ...
        r2_period=strategy.r2_period,
    )
    if 'slope' in features.columns:
        scaler = RollingStandardScaler(window=strategy.rolling_window)
        slope_norm = scaler.fit_transform(features[['slope']].values).flatten()
        features = features.copy()
        features[strategy.slope_norm_col] = slope_norm

    hmm_proba, _, _ = strategy._compute_hmm_proba(features)
    hmm_labels = np.argmax(hmm_proba, axis=1).astype(np.int64)

    # 봉 단위 라벨로 매핑 (window_end_idx에 라벨 할당)
    n_bars = len(df_test)
...
def _add_regime_shading(fig, datetimes, labels, row, col):
    """봉 단위 라벨을 같은 라벨 연속 구간으로 묶어 vrect 추가."""
...
def _plot_variant_panel(fig, variant_data, result, row, col,
    ...
    if strategy is not None:
        labels_full = _infer_regime_labels(strategy, df_test)
    else:
        labels_full = None
    ...
    labels_oos = labels_full[oos_full_idx] if labels_full is not None else None
    ...
    for i_ds in idx:
        ...
        d_v = delta_sig[i_ds]
        if labels_oos is not None and labels_oos[i_ds] >= 0:
            regime_str = REGIME_NAMES.get(int(labels_oos[i_ds]), '?')
        else:
            regime_str = '-'
...
def plot_hmm_backtest(
    ...
    fig = make_subplots(
        ...
        subplot_titles=(
            ...
            "HMM Variant A: HMM✓ Smooth✓",
            "HMM Variant B: HMM✓ Smooth✗",
            "HMM Variant C: HMM✗ Smooth✓",
            "HMM Variant D: HMM✗ Smooth✗",
        ),
