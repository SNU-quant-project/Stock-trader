...
# ─── Label Shifter (2026-07-04, smoother 대체 예정 — HANDOFF §23) ─
# Stage 1 lag 분석 결과 HMM 라벨이 실제 전환보다 수십 봉 늦음이 확인되어,
# 전환 경계를 가격 극점(zigzag/anchor)까지 당기는 label_shifter를 도입.
# ★ 기본값 'none' = 기존 동작 그대로 (live 영향 없음). Stage 3 백테스트
#   비교에서만 스크립트가 override. use_smoothed_labels와 동시 사용 불가.
#   결과 검증 + 사용자 승인 후에만 기본값 전환 예정.
LABEL_SHIFT_MODE   = 'none'   # 'none' | 'zigzag' | 'anchor'
LABEL_SHIFT_PARAMS = None     # None이면 label_shifter.py의 기본 파라미터 사용
...
LIVE_MAX_CYCLE_AGE_SEC = 600          # 시작~주문 직전 최대 허용 시간
...
LIVE_POSITION_SNAPSHOT_GUARD = True   # 이전 목표 대비 포지션 조회가 비정상적으로 비면 no-trade
LIVE_POSITION_SNAPSHOT_MIN_EXPECTED = 10
LIVE_POSITION_SNAPSHOT_MIN_COVERAGE = 0.70
LIVE_POSITION_SNAPSHOT_RETRY_COUNT = 2
LIVE_POSITION_SNAPSHOT_RETRY_SLEEP_SEC = 1.0
