...
class HMMStrategy(BaseStrategy):
    ...
    def __init__(
        ...
        # ─── Label Smoother (폐기 예정 — label_shifter로 대체, HANDOFF §23) ──
        smoother_lookback: int = 10,
        smoother_threshold: float = 0.03,
        smoother_persistence: int = 3,
        smoother_include_side: bool = False,
        # ─── Label Shifter (Stage 2~3 개선 정답지, 2026-07-04) ──────────
        # 'none'   : 시프트 없음 (기존 동작 그대로 — live 기본값)
        # 'zigzag' : 전환 경계를 독립 zigzag 극점까지 당김 (주력 후보)
        # 'anchor' : 과거 L봉 내 극점까지 당김 (비교용)
        # ★ use_smoothed_labels와 동시 사용 금지 (이중 backdate 방지).
        # ★ 당긴 라벨은 학습 타깃(y) 전용 — X/실시간 경로 유입 금지.
        label_shift_mode: str = 'none',
        label_shift_params: Optional[dict] = None,
        # ─── 메타 모델 ─────────────────────────────────────────
        meta_C: float = 1.0,
        ...
        # Label Shifter 검증 + 저장
        if label_shift_mode not in ('none', 'zigzag', 'anchor'):
            raise ValueError(
                f"label_shift_mode must be 'none'|'zigzag'|'anchor', "
                f"got {label_shift_mode!r}"
            )
        if label_shift_mode != 'none' and self.use_smoothed_labels:
            raise ValueError(
                "label_shift_mode와 use_smoothed_labels를 동시에 켤 수 없습니다 "
                "(이중 backdate). label_shift 사용 시 use_smoothed_labels=False로."
            )
        self.label_shift_mode = label_shift_mode
        self.label_shift_params = dict(label_shift_params or {})
    ...
    def from_config(cls, config_module=None, **overrides) -> 'HMMStrategy':
        ...
        params = dict(
            ...
            label_shift_mode=getattr(config_module, 'LABEL_SHIFT_MODE', 'none'),
            label_shift_params=getattr(config_module, 'LABEL_SHIFT_PARAMS', None),
    ...
    def fit(self, df: pd.DataFrame) -> 'HMMStrategy':
        ...
        if self.label_shift_mode != 'none':
            # 전환 경계를 가격 극점으로 당김 (label_shifter — HANDOFF §23).
            # fit()의 df는 학습 구간뿐이므로 OOS 확정 극점은 구조적으로 배제됨.
            close = df['close'].to_numpy(dtype=np.float64)
            end_idx = features['window_end_idx'].astype(int).values
            base_labels, shift_events = shift_labels(
                hmm_labels, close[end_idx],
                mode=self.label_shift_mode, **self.label_shift_params,
            )
            self._fit_diagnostics['label_shifts'] = sum(
                1 for e in shift_events if e['shift'] > 0)
            self._fit_diagnostics['smoother_changes'] = 0
        elif self.use_smoothed_labels:
